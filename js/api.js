const VNNUS_API = {
  ...
  async produtos() {
    ...
    return (resposta.produtos || []).map(function(produto) {
      return {
        id: produto.ID_PRODUTO || '',
        gtin: String(produto.GTIN || '').trim(),
  ...
  async produtosComEstoque() {
    ...
    const estoque = resultados[1];
    const mapaEstoque = new Map();

    estoque.forEach(function(item) {
      mapaEstoque.set(String(item.ID_PRODUTO || ''), item);
    });

    return produtos.map(function(produto) {
      const itemEstoque = mapaEstoque.get(String(produto.id));

      return {
        ...produto,
        estoque: itemEstoque
          ? Number(itemEstoque.QTD_ATUAL || 0)
          : 0,
        estoqueMinimo: itemEstoque
          ? Number(
  ...
  async produtoPorGTIN(gtin) {
    ...
    const produtos = await this.produtosComEstoque();

    return (
      produtos.find(function(produto) {
        return String(produto.gtin || '').trim() === codigo;
      }) ||
      null
    );
  },
  ...
  async dashboard() {
    ...
    return {
      ...
      estoqueCritico:
        Number(dados.estoqueBaixo || 0) +
        Number(dados.semEstoque || 0),
  ...
  async finalizarVenda(dadosVenda) {
    return await this.jsonp({
      ...
      dados: JSON.stringify(dadosVenda)
    });
  }
